...
Django под gunicorn, Next.js под node — три systemd-сервиса за nginx.

Пуш в `main` → CI → автоматическая выкатка. Вручную:

```bash
ssh root@178.105.106.8 /opt/codecup/deploy/deploy.sh
```

Установка с нуля, сертификаты, секреты и разбор сбоев —
